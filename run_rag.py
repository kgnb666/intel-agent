...
def main():
    ...
    if not args.dry_run and not emb_cfg.available:
        logger.warning("未检测到 embedding API key（环境变量 EMBEDDING_API_KEY 或 LLM_API_KEY）。")
    ...
    try:
        ...
        if not args.dry_run:
            ...
            if rows:
                logger.info("关联示例（最近 5 条有相关事件的情报）:")
                for r in rows:
                    logger.info(f"  《{r['title']}》")
                    for rel in json.loads(r["related_event"]):
                        logger.info(f"    └─ 关联 《{rel['title']}》 (相似度 {rel['similarity']})")
            trends = storage.latest_trends(3)
            if trends:
                logger.info("最新趋势脉络:")
                for t in trends:
                    logger.info(f"  [{t['week_start']}] {t['content']}")
    finally:
        storage.close()

    logger.info(f"向量化 {stats['vectorized']} 篇 | 新关联 {stats['related']} 篇 | 生成趋势 {stats['trends']} 条")
